...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!parseInt(args[0])) return client.sendTime(message.channel, `**Please choose a number between** \`1 - 500\``);
        let vol = parseInt(args[0]);
        if(vol < 0 || vol > 500){
          return  client.sendTime(message.channel, "❌ | **Please Choose A Number Between `1-500`**");
        }
        else{
        ...
        options: [
            {
                ...
                type: 4,
                required: false,
                description: "Enter a volume from 1-500. Default is 100.",
            },
        ],
    /**
     *
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!vol || vol < 1 || vol > 500) return client.sendTime(interaction, `**Please choose a number between** \`1 - 500\``);
            player.setVolume(vol);
